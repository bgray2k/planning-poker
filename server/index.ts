...
function broadcastState(room: Room) {
  for (const participant of room.participants.values()) {
    send(participant.ws, {
      type: "state",
      state: buildStateFor(room, participant.id),
    });
  }
}
...
function handleJoin(
  ...
  const shouldBeFacilitator =
    existingParticipant?.isFacilitator ?? room.participants.size === 0;
  ...
  if (room.participants.size >= MAX_PARTICIPANTS) {
    send(ws, {
      type: "error",
      message: `Room is full (max ${MAX_PARTICIPANTS} players)`,
    });
    ws.close();
    return;
  }
...
function handleReveal(room: Room, participantId: string) {
  const participant = room.participants.get(participantId);
  const hasVotes = [...room.participants.values()].some(
    (currentParticipant) => currentParticipant.vote,
  );
...
function handleMakeFacilitator(
  room: Room,
  participantId: string,
  targetId: string,
) {
  ...
  if (!participant?.isFacilitator || !target || target.id === participant.id)
    return;
...
function handleThrowEmoji(
  ...
  if (
    ...
    !REACTION_COUNTS.includes(
      (count ?? 1) as (typeof REACTION_COUNTS)[number],
    ) ||
  ...
  for (
    ...
    for (const currentParticipant of room.participants.values())
      send(currentParticipant.ws, message);
  }
}
...
httpServer.listen(PORT, () => {
  console.log(
    `Planning poker realtime server listening on ws://localhost:${PORT}`,
  );
});
