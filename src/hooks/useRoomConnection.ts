...
export function useRoomConnection(
  ...
  useEffect(() => {
    ...
    ws.addEventListener("message", (event) => {
      ...
      if (message.type === "hostTransferred") {
        setNotification(
          `${message.actorName} made ${message.targetName} the host!`,
        );
  ...
  const vote = useCallback(
    (value: CardValue) => sendMessage({ type: "vote", value }),
    [sendMessage],
  );
  ...
  const setSpectator = useCallback(
    (isSpectator: boolean) =>
      sendMessage({ type: "setSpectator", isSpectator }),
    [sendMessage],
  );
  ...
  const endSession = useCallback(
    () => sendMessage({ type: "endSession" }),
    [sendMessage],
  );
  const makeFacilitator = useCallback(
    (participantId: string) =>
      sendMessage({ type: "makeFacilitator", participantId }),
    [sendMessage],
  );
  const kickParticipant = useCallback(
    (participantId: string) =>
      sendMessage({ type: "kickParticipant", participantId }),
    [sendMessage],
  );
